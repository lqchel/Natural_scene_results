function [out] = AUC_lme(data, hypo, exp)

if hypo == 1 % do not take congruence into account
    if exp == 1
        ecc = data(data(:,2)~= -1,2);
    else
        ecc = categorical(data(data(:,2)~= -1,2));
    end
    data_table = table(ecc,data(data(:,2)~= -1,3),...
...
elseif hypo == 2
    ...
    if exp == 1
        ...
        ecc = categorical(data(data(:,3)~= -1,3));
    end

    data_table = table(ecc,data(data(:,3)~= -1,4),...
