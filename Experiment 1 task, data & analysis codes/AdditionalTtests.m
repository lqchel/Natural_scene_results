...
for loc = 1:3
    for i = 1:9
        for sub = 1:num_sub
        pcg_N(sub,i) = size(Results(Results(:,end)==location(loc)& Find_N & Results(:,9)== i-5 & Results(:,1)== subject_id(sub),:),1)/...
            size(Results(Results(:,end)==location(loc)&Find_N & Results(:,1)==subject_id(sub),:),1);
        end
    end
temp1 = reshape([pcg_N(:,1:4)],[60,1]);
temp2 = [subject_id; subject_id; subject_id; subject_id];
current_loc_matrix = [temp2 location(loc)+zeros(60,1) temp1];
if loc == 1
